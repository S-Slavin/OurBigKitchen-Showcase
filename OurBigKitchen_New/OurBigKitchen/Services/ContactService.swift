...
@MainActor
public final class ContactService: @unchecked Sendable {
    ...
    public func openEmail(to email: String) async throws {
        guard let url = URL(string: "mailto:\(email)") else { throw URLError(.badURL) }
        await UIApplication.shared.open(url)
    }
    
    public func openPhone(number: String) async throws {
        guard let url = URL(string: "tel:\(number)") else { throw URLError(.badURL) }
        await UIApplication.shared.open(url)
    }
} 