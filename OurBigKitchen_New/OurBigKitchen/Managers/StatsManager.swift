...
@globalActor actor StatsActor {
    static let shared = StatsActor()
}

@StatsActor
class StatsManager: @unchecked Sendable {
    ...
    func getUserImpact() -> AnyPublisher<Impact, Error> {
        // Return a default impact while waiting for the async task
    ...
    // Async wrapper for meal entry
    func addMealEntryAsync(_ entry: MealEntry) async throws -> DailyStats {
    ...
    // Async wrapper for volunteer registration
    func addVolunteerRegistrationAsync(_ registration: VolunteerRegistration) async throws -> DailyStats {
        return try await withCheckedThrowingContinuation { continuation in
            var cancellables = Set<AnyCancellable>()
            addVolunteerRegistration(registration)
                .sink(
                    receiveCompletion: { completion in
                        if case .failure(let error) = completion {
                            continuation.resume(throwing: error)
